...
(async () => {
	try {
		...
		for (const office of offices) {
			const dentistCount = faker.number.int({
				min: 2,
				max: 5
			});
		...
		for (const patient of patients) {
			...
			const howManyPlans: number = faker.number.int({
				min: 0,
				max: 2
			});
			...
			for (let i = 0; i < howManyPlans; i++) {
				await PatientInsurance.createRandom(patient.patient_id, shuffledPlans[i].plan_id, i === 0);
			}
		}
		...
		for (const patient of patients) {
			...
			const appointmentCount = faker.number.int({
				min: 1,
				max: 4
			});
		...
		for (const appointment of appointments) {
			...
			const procedureCount: number = faker.number.int({
				min: 1,
				max: 3
			});
		...
		for (const invoice of invoices) {
			...
			const paymentsCount = faker.number.int({
				min: 0,
				max: 2
			});
